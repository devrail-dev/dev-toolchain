#!/usr/bin/env bash
# tests/test-ansible.sh — Validate Ansible tooling installation
#
# Purpose: Verifies that all Ansible tools are installed and executable.
# Usage:   bash tests/test-ansible.sh [--help]
...
# --- Help ---
if [[ "${1:-}" == "--help" || "${1:-}" == "-h" ]]; then
  log_info "test-ansible.sh — Validate Ansible tooling installation"
  log_info "Usage: bash tests/test-ansible.sh [--help]"
  log_info "Checks: ansible-lint, molecule"
  exit 0
fi

# --- Main ---

log_info "Validating Ansible tooling installation"
...
log_info "All Ansible tools validated successfully"
